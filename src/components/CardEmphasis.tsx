  ...
  return (
    <div className="relative flex flex-col items-center justify-center text-center rounded-xl shadow-md transition-all duration-200 group hover:shadow-xl hover:scale-105">
      <div className="relative w-full h-full">
        ...
        />
        <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-start items-center text-[#0CE8F6] text-shadow-2xs p-2 sm:p-4 text-center bg-opacity-50 transition-opacity duration-300 ease-in-out group-hover:opacity-0"
          style={{ paddingTop: '58%' }}>
          <span
            className="absolute bottom-10 sm:text-3xl md:text-5xl font-bold uppercase tracking-wider mb-1 leading-none [text-shadow:2px_2px_4px_rgba(0,0,0,0.6)]">
            {name}
          </span>
          {title && (
            <p className="absolute bottom-0 md:bottom-3 mt-1 sm:mt-2 text-xs sm:text-base md:text-lg font-semibold text-gray-200 leading-tight [text-shadow:1px_1px_3px_rgba(0,0,0,0.6)]">
              {title}
            </p>
          )}
        </div>
      ...
      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center text-white p-4 sm:p-5 md:p-6 text-center opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100">
        ...
        <div className="relative z-10">
          ...
          {bio && <p className="text-lg hidden md:block font-semibold text-justify text-gray-200 leading-tight [text-shadow:1px_1px_3px_rgba(0,0,0,0.8)]">{bio}</p>}
          <br />
